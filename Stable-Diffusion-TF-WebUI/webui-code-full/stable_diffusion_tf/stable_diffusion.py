...
class Text2Image:
    def __init__(self, img_height=1000, img_width=1000,
                 model_path=None,
                 jit_compile=False, download_weights=True):
        self.img_height = img_height
        self.img_width = img_width
        self.tokenizer = SimpleTokenizer()

        if model_path is None:
            text_encoder, diffusion_model, decoder = get_models(img_height,
                                                                img_width,
                                                                download_weights=download_weights)
        else:
            text_encoder, diffusion_model, decoder = get_models_from_ui(img_height,
                                                                img_width,
                                                                model_path,
                                                                download_weights=download_weights)
...
def get_models(img_height, img_width, download_weights=True):
    ...
    if download_weights:
        ...
        text_encoder_weights_fpath = keras.utils.get_file(
            origin="/Users/avkashchauhan/miniforge3/lib/python3.10/text_encoder.h5"
        )
        diffusion_model_weights_fpath = keras.utils.get_file(
            origin="/Users/avkashchauhan/miniforge3/lib/python3.10/diffusion_model.h5"
        )
        decoder_weights_fpath = keras.utils.get_file(
            origin="/Users/avkashchauhan/miniforge3/lib/python3.10/decoder.h5"
        )
...
def get_models_from_ui(img_height, img_width, root_path, download_weights=True):
    n_h = img_height // 8
    n_w = img_width // 8
    ...
    if download_weights:
        ...
        #     file_hash="6d3c5ba91d5cc2b134da881aaa157b2d2adc648e5625560e3ed199561d0e39d5",
        # )
        text_encoder_path = root_path + "/text_encoder.h5"
        diffusion_model_path = root_path + "/diffusion_model.h5"
        decoder_weights_path = root_path + "/decoder.h5"

        text_encoder_weights_fpath = keras.utils.get_file(
            # origin="/Users/avkashchauhan/miniforge3/lib/python3.10/text_encoder.h5"
            origin=text_encoder_path
        )
        diffusion_model_weights_fpath = keras.utils.get_file(
            # origin="/Users/avkashchauhan/miniforge3/lib/python3.10/diffusion_model.h5"
